...
/**
 * The real backend (`AlbumController::show` + `AlbumLoader::load` with the
 * `albumPage` loader) has no separate `/albums/{id}/tracks` endpoint — it
 * nests the album's tracks inside the single `GET /albums/{id}` response,
 * which [Album.tracks] captures.
 */
@Serializable
